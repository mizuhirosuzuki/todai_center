...
\title{Winter weather on exam days and matriculation for a prestigious university in Japan}%: \\ Empirical analysis on auditing reports in Brazil}
...
Moreover, additional analyses suggest that the temperature in the morning on exam dates rather than the temperature during exams impact exam performance. 
...
Considering the importance of the effects the exam results can have on later life outcomes, it is important to understand how external factors such as temperature affect exam performance and what we can do to ameliorate such effects.
...
I use prefecture-level information to investigate how low temperature affects the cognitive performance of exam takers.
...
On the other hand, high-school graduates are assigned to test sites in the area where the homes used in their applications are located.\footnote{
...
(ii) allowing them to take exams in a separate room, and
(iii) allowing them to take make-up exams a week later.
The same accommodations can be given to students who could not take exams for other reasons such as sickness or accidents in public transportation system.
...
Based on hourly weather on exam days at prefecture capitals, I use average weather between 6 AM to 6 PM.
...
Let $T_{jt}$ be the average temperature across two exam days in $j$ in year $t$.
I use functions of $T_{jt}$ in the regressions ($f$), which can be linear or non-linear.
I include other weather variables such as rainfall, snowfall, and cumulated snow on the ground on exam days as a vector $X_{jt}$.
...
As Figure \ref{fig:temperature_diff} shows, each prefecture experiences substantial variations in temperature on exam days.\footnote{
  ...
  The figure shows that temperature is low for all prefectures in some years (eg. 2017) and is high in other years (eg. 2019).
...
The result shows that snow on the ground cumulated more than 10 cm ($\approx$ 4 inches) decreases the matriculation share by 0.13 percentage point.
...
I further investigate if the findings above are due to weather in the morning or during the exams.
...
Also, including both kinds of weathers simultaneously, column (5) shows the negative and significant impact of cold weather in the morning on the matriculation share.\footnote{
...
This suggests that the morning weather on the exam dates matter for exam performance.

%\begin{table}[H]
%  \center
%  \caption{Regression: Matriculation share (\%) and weather in the morning vs. during exam}
%  \scriptsize
%  \input{"../Output/tex/reg_morning_exam.tex"}
...
%      For variables during the exam, I use average weather variables at prefecture capitals between 9 AM to  on exam days.
%      Standard errors are clustered at the prefecture level.
%  \end{tablenotes}
%\end{table}

One possible interpretation of the results is that the temperature on exam days captures the weather before the exam, which can affect preparation for the exam. 
For instance, cold weather can make it hard for students to focus on studying, or snowfalls can prevent students from going to schools or cram schools for studying.
...
The results show both economically and statistically significant impacts of low temperature and cumulated snow on the ground on the exam days.
...
Using the weather in the morning and during the exams, I find suggestive evidence that the temperature effect is mainly caused by low temperatures in the morning.
...
First, the result that low temperature in the morning rather than during the exam matters suggests that investment to improve the condition at exam sites may have limited impacts.
Rather, for instance, it may be effective to provide sufficient time before the exam at exam sites so that students can recover in warm rooms.

Secondly, my results have implications for the admission process of universities.
...
\begin{table}[H]
  \center
  \caption{Regression: Matriculation share (\%) and weather on exam days by gender}
...
\begin{table}[H]
  \center
  \caption{Regression: Matriculation share (\%) and weather on exam days and one year after}
  \scriptsize
