...
for (i in seq(nrow(loc_df))) {
  ...
  write_csv(
    ...
    file.path(dropbox_dir, paste0("Data/Raw/weather_hourly/", i, "_weather_hour.csv"))
    )
...
weather_hour_all <- map(
  seq(nrow(loc_df)),
  ~ read_csv(
    file.path(dropbox_dir, paste0("Data/Raw/weather_hourly/", ., "_weather_hour.csv"))
    )
  ) %>% 
  bind_rows() %>% 
  mutate(
...
write_csv(
  weather_hour_all, 
  file.path(dropbox_dir, "Data/Raw/weather_hourly", "all_weather_hour.csv")
  )
...
for (i in seq(nrow(loc_df))) {
  ...
  write_csv(
    weather_hourly_output, 
    file.path(dropbox_dir, paste0("Data/Raw/weather_hourly_f1/", i, "_weather_hour_f1.csv"))
    )
...
weather_hour_all_f1 <- map(
  seq(nrow(loc_df)),
  ~ read_csv(
    file.path(dropbox_dir, paste0("Data/Raw/weather_hourly_f1/", ., "_weather_hour_f1.csv"))
    )
  ) %>% 
  bind_rows() %>% 
  mutate(
...
write_csv(
  weather_hour_all_f1, 
  file.path(dropbox_dir, "Data/Raw/weather_hourly_f1", "all_weather_hour_f1.csv")
  )
...
for (i in seq(nrow(loc_df))) {
  ...
  write_csv(
    weather_day_output, 
    file.path(dropbox_dir, paste0("Data/Raw/weather_day/", i, "_weather_day.csv"))
    )
...
weather_day_all <- map(
  seq(nrow(loc_df)),
  ~ read_csv(
    file.path(dropbox_dir, paste0("Data/Raw/weather_day/", ., "_weather_day.csv"))
    )
  ) %>% 
  bind_rows() %>% 
  mutate(
...
write_csv(
  weather_day_all, 
  file.path(dropbox_dir, "Data/Raw/weather_day/", "all_weather_day.csv")
  )
